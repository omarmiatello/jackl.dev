...
object EsselungaClient {
    ...
    fun getAvailableSlots(): List<Slot> {
        ...
        val response = retryNotNull {
            val call = get("https://www.esselungaacasa.it/ecommerce/resources/auth/slot/status") {
            ...
            try {
                json.parse(SlotResponse.serializer(), body)
            } catch (e: Exception) {
                null
            }
        }
    ...
    private fun esselungaKeys(): Keys? {
        val loginStep1 = postFormUrlEncoded(
        ...
        val loginStep2 = get(loginStep1.headerFields["location"]!!.first()) {
        ...
        return if (loginLocation3 != null) {
            val loginStep3 = get(loginLocation3) {
